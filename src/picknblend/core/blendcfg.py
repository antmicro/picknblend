...
def open_blendcfg(path: str, config_preset: str) -> Dict[str, Any]:
    ...
    if not config_preset:
        if "default" not in project_cfg:
            raise RuntimeError(f"Default config is not defined in YAML config file.")
...
def copy_blendcfg(file_path: str, pnb_path: str) -> None:
...
def merge_blendcfg(file_path: str, pnb_path: str, overwrite: bool = False) -> None:
    """
    ...
    template_cfg_path = pnb_path + "/templates/" + BLENDCFG_FILENAME
